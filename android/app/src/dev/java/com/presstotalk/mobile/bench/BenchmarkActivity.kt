...
class BenchmarkActivity : Activity() {
    ...
    private fun loadClips(): List<Clip> =
        benchDir().listFiles { f -> f.extension.equals("wav", ignoreCase = true) }
            ?.sortedBy { it.name }
            ?.mapNotNull { file ->
                runCatching { Clip(file.name, readMono16kWav(file)) }
    ...
    /**
     * Minimal 16-bit PCM WAV reader. Chunks are walked rather than assumed at
     * fixed offsets, because some encoders insert LIST/fact chunks before data.
     */
    private fun readMono16kWav(file: File): FloatArray {

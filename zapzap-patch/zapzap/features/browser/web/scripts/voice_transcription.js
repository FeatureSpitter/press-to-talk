...
(() => {
    ...
    function mutationsPaused() {
        return pauseDepth > 0;
    }

    function pauseMutations(ms = 6000) {
        pauseDepth += 1;
        setTimeout(() => {
    ...
    function scheduleScan() {
        if (scanScheduled || mutationsPaused()) return;
        scanScheduled = true;
        setTimeout(() => {
            scanScheduled = false;
    ...
    function installInteractionGuards() {
        for (const eventName of ["paste", "drop", "cut"]) {
            document.addEventListener(eventName, () => pauseMutations(15000), true);
        }
        document.addEventListener("keydown", (event) => {
            if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "v") {
                pauseMutations(15000);
            }
            if (event.key === "Enter" || event.key === "Backspace" || event.key.length === 1) {
                pauseMutations(3000);
            }
        }, true);
        document.addEventListener("input", () => pauseMutations(3000), true);
        document.addEventListener("focusin", (event) => {
            ...
            if (
                ...
                target.closest('[data-testid="conversation-compose-box-input"]')
            ) {
                pauseMutations(5000);
            }
        }, true);

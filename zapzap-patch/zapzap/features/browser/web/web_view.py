...
from zapzap.features.voice_transcription.voice_transcription_service import VoiceTranscriptionService
...
class WebView(QWebEngineView):
    ...
    def __init__(self, user: User = None, page_index=None, parent=None):
        ...
        self._voice_transcription_js = None
    ...
    def _inject_voice_transcription(self):
        """Inject Press-to-Talk voice message transcription into WhatsApp Web."""
        self._voice_transcription_js = self._build_voice_transcription_js()
        try:
            script = QWebEngineScript()
            script.setName("press_to_talk_voice_transcription")
            script.setInjectionPoint(
                QWebEngineScript.InjectionPoint.DocumentReady)
            script.setRunsOnSubFrames(False)
            script.setSourceCode(self._voice_transcription_js)
            script.setWorldId(QWebEngineScript.ScriptWorldId.MainWorld)
            self.profile.scripts().insert(script)
        except Exception as e:
            print(f"Error injecting voice transcription: {e}")

    def _build_voice_transcription_js(self) -> str:
        placeholders = {
            "{qwebchannel_js_code}": self._get_web_channel_js_code(),
        }

        base_dir = os.path.dirname(__file__)
        js_path = os.path.join(base_dir, "scripts", "voice_transcription.js")
        file = QFile(js_path)
        if not file.open(QFile.OpenModeFlag.ReadOnly):
            raise RuntimeError(file.errorString())
        js_code = QTextStream(file).readAll()
        ...
        pattern = re.compile("|".join(re.escape(key)
                             for key in placeholders.keys()))
        return pattern.sub(lambda m: placeholders[m.group(0)], js_code)

    def _run_voice_transcription_bootstrap(self) -> None:
        """Re-run JS bootstrap after WhatsApp Web finishes loading."""
        if not self.whatsapp_page or not self._voice_transcription_js:
            return
        bootstrap = (
            "(function(){"
            "try { window.__pttBoot && window.__pttBoot(); }"
            "catch(e){ console.error('[Press-to-Talk]', e); }"
            "})();"
        )
        self.whatsapp_page.runJavaScript(bootstrap)

    def _setup_web_channel(self) -> None:
        voice_service = VoiceTranscriptionService(self)
        self._voice_transcription = voice_service
        ...
        class ZapZapBridge(QObject):
            ...
            @pyqtSlot(str, str)
            def requestTranscription(self, msg_id: str, base64_data: str):
                voice_service.request_transcription(msg_id, base64_data)

            @pyqtSlot(str)
            def checkCache(self, msg_id: str):
                cached = voice_service.get_cached_transcript(msg_id)
                if cached:
                    voice_service._deliver(msg_id, cached)

            @pyqtSlot(str)
            def pttLog(self, message: str):
                voice_service.log(message)
    ...
    def _setup_page(self):
        ...
        self._inject_voice_transcription()
    ...
    def _on_load_finished(self, success):
        ...
        if not success:
            ...
            return
        QTimer.singleShot(1500, self._run_voice_transcription_bootstrap)
